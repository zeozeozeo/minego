...
func main() {
	packetIDsImport := flag.String("packet-ids-import", "github.com/zeozeozeo/minego/internal/data/versions/v26_2/packet_ids", "packet_ids package import path")
	flag.Parse()
	dir := "."
	if flag.NArg() > 0 {
		dir = flag.Arg(0)
	...
	packets = filterKnownPacketIDs(packets, filepath.Join(filepath.Dir(dir), "packet_ids"))
...
func filterKnownPacketIDs(packets []packetInfo, packetIDsDir string) []packetInfo {
	known := map[string]bool{}
	entries, _ := os.ReadDir(packetIDsDir)
	identifier := regexp.MustCompile(`\b(C2S|S2C)[A-Za-z0-9_]+ID\b`)
	for _, entry := range entries {
		if entry.IsDir() || !strings.HasSuffix(entry.Name(), ".go") {
			continue
		}
		data, err := os.ReadFile(filepath.Join(packetIDsDir, entry.Name()))
		if err != nil {
			continue
		}
		for _, name := range identifier.FindAllString(string(data), -1) {
			known[name] = true
		}
	}
	filtered := packets[:0]
	for _, packet := range packets {
		if known[packet.idConst] {
